...
class RevIN(nn.Module):
    def __init__(self, num_features: int, eps=1e-5, affine=True, subtract_last=False, last_n=1):
        """
    ...
    def _get_statistics(self, x):
        ...
        if self.subtract_last:
            last_n = min(self.last_n, x.shape[1])
            self.last = x[:,-last_n:,:]
            if last_n == 1:
                self.last.unsqueeze(1)
            else:
                self.last = torch.mean(self.last, dim=dim2reduce, keepdim=True).detach()
        else:
